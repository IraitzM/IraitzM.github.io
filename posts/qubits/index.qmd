...
What is quantum computing? Well, nothing but a new paradigm to perform computation. It is based on the formalism of **quantum physics** and even though it might be challenging to grasp at first, just some basic linear algebra might help on understanding the basics. The rest, is pure engineering.

Classical computing also embraces physics but at a much higher abstraction level. Boolean algebra made possible an universal set of operations to produce the algorithms we do use on a daily basis. Then, being able to manufacture bits as voltages made possible to build machines capable of following that boolean algebra. The rest is history.

The main idea behind quantum computing is that, instead of working at a macroscopic level as we are used to, by using quantum states on some physical mean we could benefit from those effects inherent to quantum physics. Be leveraging the these lower level effects some really complex calculations could be performed more efficiently. _Superposition_, _interference_ and _entanglement_ are key to most of the speedups claimed in the literature, and we will try to understand how we can work with those.

This is part of the recent history of the field and future roadmap.
<figure markdown>
  ![QC timeline](timeline.png)
</figure>

The main concept leveraged by quantum computers is the ability to act over quantum states. A quantum state is no other thing than a mathematical description of the probability distribution for the potential outcome after measurement upon a system. It sounds tricky but in general we could think of it as a normed vector. Let's try with the most simple example, a coin. 

Well, the quantum version of a coin looks like:
...
The state is a superposed state, no questions on that, but our ability to perceive it is limited by the measurement procedure itself. We need to project our quantum state into one of the two potential states we can detect ($|0\rangle$ or $|1\rangle$). Sadly, when measuring only classical bits can be obtained (0 or 1) so no probability amplitude, no phase... information gets lost in the measurement process.

The _probability histogram_ is created after a sequence of quantum state creations and measurements we often call **SHOTS**.

<figure markdown>
![Coin toss](cointoss.png)
</figure>
...
<figure markdown>
![Bloch sphere](bloch.gif){ width=60% }
</figure>
...
I think is enough for today but keep an eye, I will continue some of this explanations.